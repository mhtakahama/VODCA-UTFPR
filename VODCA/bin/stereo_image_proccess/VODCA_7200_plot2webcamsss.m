...
%% CONVERT TO A ACELLERATION

for i=1:L-1
    dummy(i,:)=(point3d(i+1,:)-point3d(i,:))/(tempo(i+1)-tempo(i)); %normalização do deslocamento
end
...
%     end
    ...
    set(gcf, 'units','normalized','outerposition',[0 0 1 1]);%Maximiza a janela
    set(gcf,'Name',['Plot number ' num2str(plot_number)],'NumberTitle','off');
    graph_plot=plot(jan*max(desl_butter))
    hold on
